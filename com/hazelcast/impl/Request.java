...
//        Integer cur = Const.version.get();
        Integer Val = (Integer) toObject(value);
        if (operation.equals(ClusterOperation.CONCURRENT_MAP_PUT))
//        if (!cur.equals(Val))
            System.out.println(operation.toString() + Val);
        else
            System.out.println(operation.toString() + Val);
       // RootLogger.getLogger(Request.class).log(Level.OFF, Val  + " putting ");
        this.local = local;
